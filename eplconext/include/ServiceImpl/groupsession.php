...
class Service_groupsession extends EPLc_Service_IAbstractService {
	...
	function perform($userinfo, $groupinfo, $serviceargs) {
        ...
        error_log("USERINFO->USERID: " . $userinfo->_userdata['userinfo']->_userId);
        error_log("USERINFO->COMMONNAME: " . $userinfo->_userdata['userinfo']->_userCommonName);
        $ep_author = $oEPLclient->createAuthorIfNotExistsFor($userinfo->_userdata['userinfo']->_userId, $userinfo->_userdata['userinfo']->_userCommonName);
